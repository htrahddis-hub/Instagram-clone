...
const Profile = (props) => {
  ...
  const followClick = () => {
    if (owner) return;
    if (!following) {
      addFollower(props.user.username, profileData._id).then((data) =>
        updateProflie(params.username)
      );
    } else {
      deleteFollower(props.user.username, profileData._id).then((data) =>
        updateProflie(params.username)
      );
    }
  };
    ...
    <div className="mt-5 pt-5">
      ...
      <div className="d-flex justify-content-center">
        <div className="pe-5 fit-content">
          <img
            src={profileData.photo ? profileData.photo.asset.url : "/80.png"}
            width="300px"
          />
        </div>
        <div>
      ...
      <div className=" mt-5 mb-5 py-5 mx-md-5 px-md-5 mx-sm-3 mx-lg-5 px-lg-5 border bg-light">
        <div className="row row-cols-1 row-cols-md-2 row-cols-sm-2 row-cols-lg-3 g-4 bd-highlight">
          {posts && posts.length > 0
            ? posts.map((post, idx) => {
                return (
                  <div className="col">
                    <div className="card h-100 postimage">
                      <img className="my-auto" src={post.photo.asset.url} key={idx} />
                    </div>
                  </div>
                );
              })
